...
export default function Scored({
  ...
  const { data: wallet, error } = useSWR(
    blockchains.find((b) => b.slug === blockchainSlug).group === "eco"
      ? `${apiHost}/api/v1/${blockchain}/wallet/${fullAddress}/score?ScoreType=1&TokenAddress=${ecoTokenAddress}&UseTokenLists=false&GetHoldTokensBalances=false`
      : `${apiHost}/api/v1/${blockchain}/wallet/${fullAddress}/score?ScoreType=0&UseTokenLists=false&GetHoldTokensBalances=false`,
    fetcher
  );
