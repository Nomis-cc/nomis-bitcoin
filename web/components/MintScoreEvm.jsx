...
export default function MintScore(props) {
  ...
  async function fetchMyScore(address, blockchain, apiHost) {
    ...
    if (switchResult){
      ...
      await fetch(
        isTokenScore
          ? `${apiHost}/api/v1/${blockchain}/wallet/${address}/score?Nonce=${nonce}&Deadline=${deadline}&ScoreType=1&TokenAddress=${props.ecoTokenAddress}&UseTokenLists=false&GetHoldTokensBalances=false`
          : `${apiHost}/api/v1/${blockchain}/wallet/${address}/score?Nonce=${nonce}&Deadline=${deadline}&ScoreType=0&UseTokenLists=false&GetHoldTokensBalances=false`
      )
      .then((res) => res.json())
      .then(
        (result) => {
